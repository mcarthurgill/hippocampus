class ItemsController < ApplicationController
  ...
  def index
    ...
    @users_added_item_last_24_hours = Item.where("created_at > ? AND user_id NOT IN (?)", 1.day.ago, [23, 2, 18, 15, 81, 189, 190, 191]).pluck(:user_id).uniq.count
    @users_added_item_last_week = Item.where("created_at > ? AND user_id NOT IN (?)", 7.days.ago, [23, 2, 18, 15, 81, 189, 190, 191]).pluck(:user_id).uniq.count
