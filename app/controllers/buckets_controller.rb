class BucketsController < ApplicationController
  ...
  def info
    ...
    @items = @bucket.items.not_deleted.by_date.for_page_with_limit(page, 15).reverse if @bucket
    @item = Item.new
  ...
  def detail
    ...
    if params[:id].to_i == 0
      ...
      item_keys = user.items.outstanding.by_date.pluck(:local_key)+user.bucket_items.by_date.not_deleted.limit(2000).pluck(:local_key).uniq
