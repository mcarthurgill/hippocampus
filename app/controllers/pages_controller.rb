class PagesController < ApplicationController
  ...
  def search
    ...
    if params.has_key?(:t) && params[:t].strip.length > 0
      ...
      @results = client.search('engine', params[:t], {:document_types => ['items','buckets'], :search_fields => { 'items' => ['text^4','buckets_string'], 'buckets' => ['first_name'] }, :filters => { 'items' => {'available_to' => {"type"=>"and","values"=>[user_id]} }, 'buckets' => {'available_to' => {"type"=>"and","values"=>[user_id]} } } } )
    end
